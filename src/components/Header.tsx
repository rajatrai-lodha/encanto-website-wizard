      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-6">
          ...
          <Link to="/" className="flex items-center space-x-6 group">
            ...
            <div className="hidden sm:block bg-white/95 backdrop-blur-sm rounded-xl px-4 py-2 shadow-sm border border-white/20">
          ...
          <nav className="hidden lg:flex items-center space-x-2">
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`relative px-6 py-3 rounded-xl text-sm font-semibold transition-all duration-500 ease-out overflow-hidden group ${
                  isActive(item.href)
                    ? 'bg-white text-primary shadow-md font-bold border-2 border-primary/20 transform scale-105'
                    : 'text-foreground hover:text-primary hover:bg-white/20 hover:shadow-lg hover:scale-102'
                }`}
              >
                ...
                <div className={`absolute bottom-1 left-1/2 h-0.5 bg-primary transition-all duration-500 ease-out ${
                  isActive(item.href) 
                    ? 'w-3/4 transform -translate-x-1/2' 
                    : 'w-0 group-hover:w-3/4 transform -translate-x-1/2'
                }`} />
          ...
          <div className="lg:hidden">
            <Button
              ...
              className="relative w-12 h-12 p-0 hover:bg-primary/10 transition-all duration-300"
            >
          ...
          <nav className="flex flex-col space-y-3">
            {navigation.map((item, index) => (
              <Link
                ...
                className={`relative px-6 py-4 rounded-xl text-sm font-semibold transition-all duration-500 ease-out overflow-hidden group ${
                  isActive(item.href)
                    ? 'bg-white text-primary shadow-md font-bold border-2 border-primary/20 transform scale-105'
                    : 'text-foreground hover:text-primary hover:bg-white/20 hover:shadow-lg hover:scale-102'
                }`}
              ...
              >
                ...
                <div className={`absolute bottom-2 left-6 h-0.5 bg-primary transition-all duration-500 ease-out ${
                  isActive(item.href) 
                    ? 'w-3/4' 
